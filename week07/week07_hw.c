...
void sort_list(tNumStorHead *list, int input)
{
    ...
    if (list->head == NULL)
    {
        new_storage->next = NULL; // fix: new_storage->next = list->head; 
    ...
    else
    {
        ...
        while (tmp_ptr != NULL)
        {
            if (input <= tmp_ptr->number && tmp_ptr == list->head) // fix: if (input <= tmp_ptr->number)
            {
                new_storage->next = tmp_ptr;
                tmp_ptr->prev = new_storage; // add
                new_storage->prev = tmp_ptr->prev;
                list->head = new_storage;
                break;
            }
            else if (input >= tmp_ptr->number && tmp_ptr == list->tail) // fix: else if (input > tmp_ptr->number && tmp_ptr == list->tail)
            {
